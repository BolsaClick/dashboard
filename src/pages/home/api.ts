import { NextApiRequest, NextApiResponse } from 'next';

// Função para buscar dados da comissão
export const fetchCommissionData = async () => {
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..n9EdLPzIEXTlknBY2Dwpax_h3rbNXZWrke9QoI8T_gE';

  try {
    const response = await fetch('https://api.consultoriaeducacao.app.br/commission/listInstallmentsByPromoterDC/6716698cb4d33b0008a18001?limit=999999&page=1', {
      method: 'GET',
      headers: {
        'Authorization': `${token}`,
      },
    });

    if (!response.ok) {
      throw new Error('Erro ao buscar dados');
    }

    const data = await response.json();
    return data.data;
  } catch (error: any) {
    throw new Error(error.message);
  }
};

// Função handler que será chamada pela API
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    const commissionData = await fetchCommissionData(); // Chama a função que faz a requisição
    res.status(200).json(commissionData); // Retorna os dados como resposta
  } catch (error) {
    res.status(500).json({ error: 'Erro ao buscar dados' }); // Retorna erro em caso de falha
  }
}
